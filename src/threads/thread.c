               ...
               thread_func *function, void *aux)
{
  ...
  t->blocked_ticks = 0;//创建线程时 阻塞时间初始化为0
...
/* Puts the current thread to sleep.  It will not be scheduled
  ...
  将当前线程sleep阻塞直到被thread_unblocked唤醒之前都不会被拉来跑
...
thread_block (void)
{
  ASSERT (!intr_context ());//不是外中断 IO中断
  ASSERT (intr_get_level () == INTR_OFF);//过程中不能被中断

  thread_current ()->status = THREAD_BLOCKED;//当前线程 阻塞
  schedule ();//从队列中挑选下一个线程跑
}
...
//函数：修正线程阻塞时间函数
void
thread_revise_blocked_ticks(struct thread *threadin,void *aux)
{
  ASSERT (intr_get_level () == INTR_OFF);
  if(threadin->status == THREAD_BLOCKED)//阻塞状态
  {
